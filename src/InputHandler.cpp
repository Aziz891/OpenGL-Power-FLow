...
void InputHandler::onMouseMove(int32_t mousePosX, int32_t mousePosY,
  ...
  if (isRotating) {
    // std::cout << deltaX << " , " << deltaY <<  " , " << " , " << mousePosX <<
    // " , "  << mousePosY << std::endl ; std::cout << _transform.GetRot()->x <<
    // " >> ";
    _transform.GetRot()->y += 0.01 * deltaX;
    _transform.GetRot()->x -= 0.01 * deltaY;

    std::cout << _transform.GetRot()->x << ", " << _transform.GetRot()->y
              << std::endl;

  } else if (isPanning){
    _transform.GetPos()->x -= 0.1 * deltaX;
    _transform.GetPos()->y -= 0.1 * deltaY;
  }
}
...
// void GameEventHandler::addKeyControl(uint32_t keyCode, InputControl&
// inputControl, float weight)
// {
// }
// void GameEventHandler::addMouseControl(uint32_t mouseButton, InputControl&
// inputControl, float weight)
// {
// }

// void GameEventHandler::updateInput(uint32_t inputCode, float dir, bool
// isRepeat)
// {
// 	if(isRepeat) {
// 		return;
...
// 	// 	inputs[inputCode][i].second.addAmt(inputs[inputCode][i].first *
// dir);
// 	// }
// }
